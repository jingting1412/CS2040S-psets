...
public class GameTree {
     ...
     */
    int findValue(TreeNode root, Player player) {
        if (root.leaf) { //if its a leaf, then return its value
            return root.value;
        } else if (player == Player.ONE) { //player one wants to max their score
            int bestValue = Integer.MIN_VALUE;
            for (TreeNode child : root.children) {
                bestValue = Math.max(bestValue, findValue(child, Player.TWO));;
            }
            root.value = bestValue;
            return bestValue;
        } else { //player two wants to min their score
    ...
    int findValue() {
        if (this.root == null) {
            ...
            return findValue(this.root, Player.ONE);
        }
    }
